...
//多麦
var web_union_id = GetQueryString("union_id");
if (web_union_id == "duomai") {
    web_from = web_union_id;
}
var web_fromparam_duomai = "from=" + web_from + "&feedback=" + GetQueryString("feedback")
                            + "&mid=" + GetQueryString("mid")
                            + "&to=" + GetQueryString("to");

//接入来源URL
if (GetQueryString("fromurl") != "") {
    //localStorage[g_const_localStorage.OrderFromRefer] = My_URL_UrlDecode.UrlDecode(GetQueryString("fromurl"));//web_refer;
    localStorage["OrderFromRefer"] = GetQueryString("fromurl");
    //记录失效时间(30天后)
    var Expires_duomai = Date.parse(new Date()) + 15 * 24 * 60 * 60 * 1000;
    localStorage["Expires_duomai"] = Expires_duomai;
}
...
var Merchant1 = {
                ...
                if (msg.resultcode) {
                    if (msg.resultcode == g_const_Success_Code) {
                        if (msg.resultmessage != localStorage[g_const_localStorage.OrderFrom] && msg.resultmessage!="") {
                            ...
                            //多麦处理--开始
                            if (web_from == "duomai") {
                                localStorage[g_const_localStorage.OrderFromParam] = web_fromparam_duomai;
                                ////接入来源URL
                                //if (GetQueryString("fromurl") != "") {
                                //    localStorage[g_const_localStorage.OrderFromRefer] = My_URL_UrlDecode.UrlDecode(GetQueryString("fromurl"));//web_refer;
                                //}
                                //接入时间
                                localStorage[g_const_localStorage.OrderFromTime] = Merchant1.GetNowTime();
                            }
                            //多麦处理--结束
                            $('.mask,.guide-app').hide();
                        }
                        ...
                        //接入来源URL
                        localStorage[g_const_localStorage.OrderFromRefer] = "";
                        //接入时间
                        localStorage[g_const_localStorage.OrderFromTime] = "";
    ...
    RecordPage: function () {
        ...
        //                                                        + "&etc_d=" + GetQueryString("etc_d");
            ...
            //多麦处理--开始
            else if (web_from == "duomai") {
                web_search = web_fromparam_duomai;
            }
            //多麦处理--结束
            else {
        ...
        try {
            if (localStorage[g_const_localStorage.OrderFrom] != "") {
                if (localStorage[g_const_localStorage.OrderFrom] == Merchant1.Code) {
                    ...
                    //跳转支付
                    OrderCreate.GoToPay(Merchant1.orderid);
            ...
            if (localStorage[g_const_localStorage.OrderFrom] != "") {
                if (localStorage[g_const_localStorage.OrderFrom] == Merchant_LKT.Code) {
                    ...
                    $.ajax({
                        ...
                        success: function (msg) {
                            //跳转支付
                            OrderCreate.GoToPay(Merchant_LKT.order_code);
                        }
                    });
                }
            }
...
var Merchant_duomai = {
    ...
    euid: "",//接入时传递的参数
    order_sn: "",//订单编号
    //suborder_sn: "",//[可选]子订单编号
    order_time: "",//用户下单时间
    click_time: "",//用户点击进入cps接口的时间
    orders_price: "",//用户实际支付的现金金额
    discount_amount: "",//优惠金额
    //promotion_code: "",//[可选]优惠券号
    is_new_custom: "0",//是否新客,1为新客,0为老客,不分新老客传0
    channel: "1",//[可选]订单渠道,0为pc，1为wap，3为ROI
    order_status: "0",//订单状态,目前-1表示无效，0表示未支付状态，其它直接给出状态描述，或者使用1、2、3这样的正整数
    referer: "",//接入接口流量来源URL
    //currency: "",//[可选]货币代码,默认为人民币(CNY)，其它币种请传入标准的货币代码
    //timezone: "",//[可选]时区代码,默认为北京时间(UTC+8)，其它时区请传入标准的时区代码， http://zh.wikipedia.org/zh/%E6%97%B6%E5%8C%BA%E5%88%97%E8%A1%A8
    ...
    goods_ta: "",//商品数量
    goods_cate: "",//商品分类编号
    goods_cate_name: "",//[可选]商品分类名称
    totalPrice: "",//商品总净金额，商品总价减去此件商品使用的优惠券/现金券等其他非实付金额
    //rate: "",//[可选]商品的佣金比例
    commission: "0",//商品的佣金金额
    //commission_type: "",//[可选]佣金分类
    //encoding: "",//[可选]字符编码，如果此次推送的数据包含中文且非UTF8编码，请设置相应的编码值。如：推送的为gbk 数据 encoding=gbk，如果数据默认为 UTF8 编码则无需设置此值
    test: "",//[可选]当传递test=1时为测试

    RecordOrder: function () {
        try {
            if (localStorage[g_const_localStorage.OrderFrom] != "" && localStorage[g_const_localStorage.OrderFromRefer]!="") {
                if (localStorage[g_const_localStorage.OrderFrom] == Merchant_duomai.Code) {

                    //获得euid
                    var referer = My_URL_UrlDecode.UrlDecode(localStorage[g_const_localStorage.OrderFromRefer]);

                    var group_temp = referer.split('&');
                    for (var i = 0; i < group_temp.length; i++) {
                        if (group_temp[i].indexOf("feedback=")>-1) {
                            euid = group_temp[i].split('=')[1];
                            break;
                        }
                    }

                    if (euid != "") {
                        var OrderFromTime = localStorage[g_const_localStorage.OrderFromTime];
                        var OrderFromRefer = localStorage[g_const_localStorage.OrderFromRefer];

                        //防止支付页面异常导致数据无法清除，此处清除数据
                        //localStorage[g_const_localStorage.OrderFrom] = "";
                        //localStorage[g_const_localStorage.OrderFromParam] = "";
                        //localStorage[g_const_localStorage.OrderFromTime] = "";
                        //localStorage[g_const_localStorage.OrderFromRefer] = "";
                        ...
                        $.ajax({
                            ...
                            url: '/Ajax/API.aspx',//目标地址
                            data: "t=" + Math.random() +
                                    "&action=duomai_cps_sendorder" +
                                    "&euid=" + euid +
                                    "&order_sn=" + encodeURIComponent(Merchant_duomai.order_sn) +
                                    "&order_time=" + encodeURIComponent(Merchant_duomai.order_time) +
                                    "&click_time=" + encodeURIComponent(OrderFromTime) +
                                    "&orders_price=" + encodeURIComponent(Merchant_duomai.orders_price) +
                                    "&discount_amount=" + encodeURIComponent(Merchant_duomai.discount_amount) +
                                    //"&promotion_code=" + Merchant_duomai.promotion_code +
                                    "&is_new_custom=" + encodeURIComponent(Merchant_duomai.is_new_custom) +
                                    "&channel=" + encodeURIComponent(Merchant_duomai.channel) +
                                    "&order_status=" + encodeURIComponent(Merchant_duomai.order_status) +
                                    "&referer=" + encodeURIComponent(OrderFromRefer) +
                                    "&goods_id=" + encodeURIComponent(Merchant_duomai.goods_id) +
                                    "&goods_name=" + encodeURIComponent(Merchant_duomai.goods_name) +
                                    "&goods_price=" + encodeURIComponent(Merchant_duomai.goods_price) +
                                    "&goods_ta=" + encodeURIComponent(Merchant_duomai.goods_ta) +
                                    "&goods_cate=" + encodeURIComponent(Merchant_duomai.goods_cate) +
                                    "&goods_cate_name=" + encodeURIComponent(Merchant_duomai.goods_cate_name) +
                                    "&totalPrice=" + encodeURIComponent(Merchant_duomai.totalPrice) +
                                    "&commission=" + encodeURIComponent(Merchant_duomai.commission),
                            beforeSend: function () { },//发送数据之前
                            complete: function () {
                            ...
                            success: function (msg) {
                                ...
                                //localStorage[g_const_localStorage.OrderFromRefer] = "";
                                //跳转支付
                                OrderCreate.GoToPay(Merchant_duomai.order_sn);
...
var My_URL_UrlDecode = {
        ...
        for (var i = 0; i < str.length; i++) {
            ...
            if (chr == "+") {
                ...
                if (parseInt("0x" + asc) > 0x7f) {
                    ...
                    ret += My_URL_UrlDecode.asc2str(parseInt("0x" + asc + str.substring(i + 4, i + 6)));

                    i += 5;

                } else {

                    ret += My_URL_UrlDecode.asc2str(parseInt("0x" + asc));
    ...
    str2asc: function (str) {
        var n = My_URL_UrlDecode.UnicodeToAnsi(str.charCodeAt(0));
        var s = n.toString(16);
        return s.toUpperCase();
    },

    asc2str: function (code) {
        var n = My_URL_UrlDecode.AnsiToUnicode(code);
    ...
    UnicodeToAnsi: function (chrCode) {
        ...
        var i = My_URL_UrlDecode.UnicodeChr().indexOf(chrHex);
        if (i != -1) {
            chrHex = My_URL_UrlDecode.AnsicodeChr().substr(i, 4);
        }
    ...
    AnsiToUnicode: function (chrCode) {
        ...
        chrHex = "000" + chrHex.toUpperCase();
        chrHex = chrHex.substr(chrHex.length - 4);
        var i = My_URL_UrlDecode.AnsicodeChr().indexOf(chrHex);
        if (i != -1) {
            chrHex = My_URL_UrlDecode.UnicodeChr().substr(i, 4);
        }
        return parseInt(chrHex, 16)
    },
...
/**
    * 请求时间与当时时间比较
    * $ChaekType:"<"请求时间是否小于当前时间，“>”请求时间是否小于当前时间
   */
var My_DateCheck = {
    //JS获取本地时间
    CheckEX: function (ChaekType, CheckTime) {
        var res = false;
    ...
    //传入本地时间
    CheckEX_1: function (ChaekType,nowTime, CheckTime) {
        var res = false;
        //var nowTime = Date.parse(new Date());
        //CheckTime = Date.parse(CheckTime);
        switch (ChaekType) {
...
try {
    //判断是多麦CPS否到达失效时间
    //判断是多麦CPS否到达失效时间
    if (!(localStorage["OrderFrom"] == undefined) && localStorage["OrderFrom"] == "duomai") {
        if (My_DateCheck.CheckEX("<=", localStorage["Expires_duomai"])) {
            localStorage["OrderFrom"] = "";
        }
    }
}
catch (e) { }
...
//读取静态JS文件方法
var My_LoadDataStaitc = {
    /*读取静态文件
        JsObj:静态文件对象
        OKCallBack：返回结果正常时的回调方法
        ErrCallBack：返回结果异常，时的回调方法【调用接口查询方法】
        okData:数据OK时，需要传给回调参数的，例如Category.Load_Result(msg.scs);okdata=JsObj.scs
        IsNoData:数据OK时，回调函数是否需要传递参数，true，false
    */
    LoadStaitc: function (JsObj,OKCallBack,ErrCallBack,okData,IsNoData) {
        var msg = JsObj;
        try {
            if (msg.resultCode == g_const_Success_Code) {
