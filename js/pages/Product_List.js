...
var InnerBuyPrice = {
    GetList: function (codeList, callback) {
        ...
        var api_input = { "version": 1, "code": codeList, "memberCode": "", "areaCode": "", "sourceCode": "", "isPurchase": 0 };
...
var Product = {
    ...
    api_input: { "categoryOrBrand": "", "screenWidth": "", "pageNo": "", "pageSize": "", "sortType": "", "baseValue": "", "keyWord": "", "sortFlag": "", "channelId": g_const_ChannelID },
    GetList: function () {
    ...
    Load_Data: function () {
        if (_productData.number == "1") {
            var codeList = "";
            $(_productData.item).each(function () {
                if (codeList == "") {
    ...
    Load_Result: function (innerRes) {
        if (innerRes) {
            if (innerRes.resultCode == g_const_Success_Code) {
                $(innerRes.map).each(function () {
                    for (var i = 0; i < _productData.item.length; i++) {
                        if (_productData.item[i].currentPrice != this[_productData.item[i].productCode]) {
                            _productData.item[i].currentPrice = this[_productData.item[i].productCode];
        ...
        var bodyAction = "";
        var actnum = 0;
        $.each(_productData.item, function (i, n) {
            bodyAction = "";
            actnum = 0;
            if (_showType != "0") {
                if (!(n.activityList == undefined)) {
            ...
            //    if (label) {
            //        bodyList += '<img class="d_add_ys_lt" src="' + label.spicture + '" alt="" />';
            //    }
            //}
            //3.9.4 从接口获取图片
            if (n.labelsPic != "" && !(n.labelsPic == undefined)) {
                bodyList += '<img class="d_add_ys_lt" src="' + n.labelsPic + '" alt="" />';
            }
