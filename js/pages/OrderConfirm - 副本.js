...
$(document).ready(function () {
   ...
   // localStorage.setItem(g_const_localStorage.OrderConfirm, JSON.stringify({ "GoodsInfoForAdd": [{ "sku_num": 2, "area_code": "", "product_code": "120903", "sku_code": "120903" }] }))
    ...
    if (localStorage.getItem(g_const_localStorage.OrderConfirm)) {
        _goods = JSON.parse(localStorage.getItem(g_const_localStorage.OrderConfirm)).GoodsInfoForAdd;
    }
    if (localStorage.getItem(g_const_localStorage.OrderConfirm) == "") {
    ...
    $("#btnOrderCreate").click(function () {
        ...
        if (_needVerifyIdNumber == 1 && Address_Update.api_input.idNumber=="") {
            ShowMesaage(g_const_API_Message["106001"]);
            return;
        }
...
function SetGoodUnLogin() {
    if (localStorage.getItem(g_const_localStorage.GoodsInfo)) {
        ...
        var divHtml = "";
            ...
            $("#divdisRemarks").html("<span>注：</span>新疆、西藏地区每单收24元运费，由派件员送货时向你收取");
...
var Address_Update = {
    ...
    var OrderDetail = {
        ...
        OrderConfirm: function () {
            ...
            this.api_input.area_code = _area_code;
            this.api_input.coupon_codes = _coupon_codes;
            this.api_input.goods = _goods;
            this.api_input.order_type = _order_type;
        ...
        LoadVerifyIdNumber: function (list) {
            ...
            if (list.isVerifyIdNumber=="1") {
                if (Address_Update.api_input.idNumber == "") {
                    Message.ShowAlert(g_const_API_Message[106001], "", "divAlert", "确定");
                    $("#div_noidnum").show();
                    $("#div_idnum").hide();
...
var OrderCreate = {
    ...
    CreateToJYH: function () {
      //  alert(localStorage[g_const_localStorage.OrderFrom]);
        ...
        this.api_input.check_pay_money = _pay_money;
        this.api_input.buyer_address_id = _addressid;
        this.api_input.buyer_address_code = _area_code;
        this.api_input.goods = _goods;
        this.api_input.buyer_mobile = $("#spaddressphone").html();
        this.api_input.pay_type = _pay_type;
        this.api_input.buyer_address = _addressStreet;
        this.api_input.billInfo = _fapiao;
        this.api_input.buyer_name = $("#spaddressuser").html();
        this.api_input.order_type = _order_type;
        this.api_input.coupon_codes = _coupon_codes;
        this.api_input.order_souce = _order_souce;
        this.api_input.channelId = g_const_ChannelID;
            ...
            if (msg.resultcode) {
                if (msg.resultcode == g_const_Success_Code) {
                    ...
                    if (UserIdentity.Type==1) {
                        Message.ShowConfirm("您当前使用的身份证，通关失败，", "请修改身份证，以便顺利购物！", "divAlert", "修改身份证", "IDNumber.ChangeIDnumber", "关闭");
                    }
                    else {
                    Message.ShowConfirm("您的身份证曾被海关退回，", "若继续使用，可能再次通关失败！", "divAlert", "修改身份证", "IDNumber.ChangeIDnumber", "继续使用", "Address_Update.EditInfo");
                    }
                }
            }
        });
