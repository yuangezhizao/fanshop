...
var MyOrder_Num = {
    ...
    api_input: { "version": 1.0, "channelId": "" },//两个参数都是非必填
    GetList: function () {
        //返回参数
        ...
        MyOrder_Num.api_input.channelId = g_const_ChannelID;
        //组织提交参数
...
var MyOrder_List = {
    ...
    Load_apiSellerList: function (resultlist) {
        ...
        $.each(resultlist.apiSellerList, function (iii, nnn) {
            ...
            //}
            //3.9.4 从接口获取图片
            if (nnn.labelsPic != "" && !(nnn.labelsPic == undefined)) {
                temp += '<img class="d_add_ys" src="' + nnn.labelsPic + '" alt="" />';
            }
        ...
        $.each(resultlist.orderButtonList, function (jj, kk) {
            switch (kk.buttonCode) {
                case g_const_afterButtonCode.QXDD://取消订单
                    payShow += "<a id=\"btn_cxdd\" num='202070-3' onclick=\"btncaozuo('qxdd','" + resultlist.order_code + "','');\">取消订单</a>";
                    break;
                case g_const_afterButtonCode.FK://付款
                    payShow += "<a class=\"receipt\" num='202070-4'  onclick=\"btncaozuo('qfk','" + resultlist.order_code + "','" + resultlist.apiSellerList[0].sell_price + "');\">" + kk.buttonTitle + "</a>";
                    break;
                ...
                case g_const_afterButtonCode.CKWL://查看物流
                    payShow += "<a num='202070-8' onclick=\"btncaozuo('ckwl','" + resultlist.order_code + "','');\">" + kk.buttonTitle + "</a>";
                    break;
                case g_const_afterButtonCode.DHTH://电话退款
                    payShow += "<a id=\"btn_tksh\" num='202070-7' onclick=\"return btncaozuo('tksh','" + resultlist.order_code + "','');\">" + kk.buttonTitle + "</a>";
                    break;
                case g_const_afterButtonCode.QRSH://确认收货
                    payShow += "<a href=\"#\" onclick=\"btncaozuo('qrsh','" + resultlist.order_code + "','');\" num='202070-5' class=\"receipt\">" + kk.buttonTitle + "</a>";
                    break;
