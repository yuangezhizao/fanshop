...
var Product_Detail = {
            ...
            for (var k = 0; k < skulist.length; k++) {
                ...
                for (var kk = 0; kk < seckillskulist.length; kk++) {
                    ...
                    if (sku.skuCode.Trim() == seckillsku.skuCode.Trim()) {
                        ...
                        Product_Detail.api_response.skuList[k].stockNumSum = seckillsku.maxBuy;
    ...
    AfterLoadDetail: function (msg) {
        ...
        if (msg.labelsList.length > 0) {
            var labelsList = msg.labelsList[0];
            var pl = g_const_ProductLabel.find(labelsList);
            if (pl != null) {
                if (pl.picture.trim() != "") {
                    $("#img_gd_name").attr("src", pl.picture);
                    $("#img_gd_name").show();
                }
                else
                    $("#img_gd_name").hide();
    ...
    LoadCollection: function (msg) {
        ...
        $("#a_Collection").on("click", function (e) {
            var operate;
            if ($("#a_Collection").attr("class") == "ch01 gray")
                operate = "add";
            else
                operate = "cancel";
    ...
    /*读取价格信息*/LoadPrice: function (msg) {
        ...
        var html;
        html = '<em class="nprice"><em>￥</em>' + Product_Detail.GetSalePrice() + '</em>';
        ...
        //    var pl = g_const_ProductLabel.find(labelsList);
        //    if (pl) {
        //        if (pl.value == g_const_ProductLabel.OverSea.value) {
        //            html += guoneilabel;
        //        }
        //    }
        //}

        html += '<span style="padding:0;"><em>￥</em>' + Product_Detail.SkuKillResponse.marketPrice;
        html += '</span>';
