...
$(document).ready(function () {
    ...
    $("#btnCode").on('click', function () {
        ...
        //        return;
        //    }
        //    if (piccode.length < 6 || piccode.length > 16 || piccode.indexOf(' ') > -1) {
        //        ShowMesaage(g_const_API_Message["7904"]);
        //        return;
        //    }
        //}
...
var Recommend = {
    ...
    SetParent: function () {
        ...
        var request = $.ajax({
            ...
            cache: false,
            method: g_APIMethod,
            data: "t=" + Math.random() + "&action=setrecom&phoneno=18019190823",
            dataType: "json"
        });
        request.done(function (msg) {
            Message.Operate('', "divAlert");
            if (msg.resultcode == g_const_Success_Code) {
                ShowMesaageCallback(Recommend.RegMsg + g_const_API_Message["100033"], CloseAct, 3000);
            }
            else {
                if (msg.resultmessage.indexOf("已有上级") > 0) {
                    ShowMesaageCallback(Recommend.RegMsg + g_const_API_Message["100046"], CloseAct, 3000);
                }
                else {
                    ShowMesaageCallback(Recommend.RegMsg + msg.resultmessage, CloseAct, 3000);
                }
